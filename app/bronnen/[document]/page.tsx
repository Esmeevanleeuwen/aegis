      ...
      <div className={styles.layout}>
        ...
        <article className={styles.reading}>
          {document.pages.map((page) => (
            <section key={page.id} className={styles.sourcePage} id={pageAnchor(document.slug, page.pageNumber)}>
              <header><strong>Bronpagina {page.pageNumber}</strong><a href={`#${pageAnchor(document.slug, page.pageNumber)}`}>Link naar deze bronpagina</a></header>
              <div>{page.text || "Voor deze pagina is geen tekstextractie beschikbaar."}</div>
              <small>Controlemarkering in de bronregistratie: {page.reviewStatus || "niet opgegeven"}. Dit is geen oordeel over alle beweringen op deze pagina.</small>
            </section>
          ))}
        </article>
      </div>
