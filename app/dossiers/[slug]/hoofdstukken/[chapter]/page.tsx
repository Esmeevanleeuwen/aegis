...
export async function generateMetadata({ params }: Props) {
  ...
  return dossier && current
    ...
    : { title: "Dossierstap niet gevonden", robots: { index: false } };
}
    ...
    <Shell>
      ...
      <header className={styles.hero}>
        <p className={styles.eyebrow}>
          {current.eyebrow || `Dossierstap ${String(index + 1).padStart(2, "0")}`}
        </p>
        <h1>{current.title}</h1>
        <Link href={dossierPath(slug)}>Terug naar het politieke overzicht →</Link>
      </header>
      ...
      <div className={styles.layout}>
        ...
        <article className={styles.reading}>
          ...
          <nav className={styles.pager} aria-label="Vorige en volgende dossierstappen">
            {previous && (
          ...
          <Link href={`${dossierPath(slug)}#politieke-afweging`}>
            Ga van deze analyse naar keuze en uitvoering →
          </Link>
