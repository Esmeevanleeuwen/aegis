...
export async function generateMetadata({ params }: Props) {
  ...
  return theme ? pageMetadata(`Dossiers over ${theme.title.toLowerCase()}`, `Dossiers, hoofdstukken en bronnen over ${theme.title.toLowerCase()}.`, `/themas/${theme.slug}`, theme.dossiers.some((dossier) => dossier.indexable)) : { title: "Thema niet gevonden", robots: { index: false } };
}
  ...
  return (
    <Shell>
      ...
      <header className={styles.hero}>
        ...
        <h1>{theme.title}</h1>
        <p>Deze dossiers benaderen {theme.title.toLowerCase()} vanuit verschillende vragen. Vergelijk de onderbouwing en politieke afweging voordat je conclusies verbindt.</p>
      </header>
