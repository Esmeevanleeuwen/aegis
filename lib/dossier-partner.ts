...
const catalogue = cache(async (): Promise<DossierSummary[]> => {
  try {
    ...
    if (!data || typeof data !== "object" || !("version" in data) || data.version !== 1 || !("items" in data) || !Array.isArray(data.items)) return [];
    return data.items.slice(0, 1000).flatMap((item: unknown) => {
      if (!item || typeof item !== "object") return [];
      const row = item as Record<string, unknown>;
      if (typeof row.slug !== "string" || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(row.slug) || typeof row.title !== "string" || typeof row.description !== "string" || !Array.isArray(row.themes)) return [];
      return [{ slug: row.slug, title: row.title.slice(0, 240), description: row.description.slice(0, 500), themes: row.themes.filter((value): value is string => typeof value === "string").slice(0, 12), status: "Openbaar dossier", indexable: false }];
    });
  } catch {
    return [];
  }
